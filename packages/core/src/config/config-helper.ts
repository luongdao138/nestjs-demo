...
import { PartialPiomConfig, RuntimePiomConfig } from './piom-config'

let activeConfig: RuntimePiomConfig
 ...
 */
export async function setConfig(userConfig: PartialPiomConfig) {
  if (!activeConfig) {
 ...
 */
export function getConfig(): Readonly<RuntimePiomConfig> {
  if (!activeConfig) {
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      activeConfig = require(defaultConfigPath).defaultConfig
